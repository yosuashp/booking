...
class ManageSpaceController extends FrontendController
{
    ...
    public function restore($id)
    {
        ...
        $user_id = Auth::id();
        $query = $this->spaceClass::onlyTrashed()->where("author_id", $user_id)->where("id", $id)->first();
        if(!empty($query)){
            $query->restore();
    ...
    public function deleteSpace($id)
    {
        ...
        if(\request()->query('permanently_delete')){
            $query = $this->spaceClass::where("author_id", $user_id)->where("id", $id)->withTrashed()->first();
            if (!empty($query)) {
                $query->forceDelete();
            }
        }else {
