    ...
    public function search($request)
    {
        ...
        if (is_array($terms) and !empty($terms = array_filter(array_values($terms)))) {
            foreach ($terms as $index=>$termId){
                $model_car->join('bravo_car_term as tt'.$index, function($join) use ($termId,$index){
                    $join->on('tt'.$index.'.target_id', "bravo_cars.id");
        ...
        switch ($orderby){
            ...
            default:
                if(!empty($request['order']) and !empty($request['order_by'])){
                    $model_car->orderBy("bravo_cars.".$request['order'], $request['order_by']);
                }else{
        ...
        return $model_car->with(['location','hasWishList','translation']);
    }
