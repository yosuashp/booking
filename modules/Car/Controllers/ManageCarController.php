...
class ManageCarController extends FrontendController
{
    ...
    public function __construct(Car $carClass,CarTranslation $carTranslationClass, Location $locationClass,Attributes $attributesClass, CarTerm $carTermClass,Booking $bookingClass, Enquiry $enquiryClass)
    {
        parent::__construct();
        $this->locationClass = $locationClass;
        $this->carClass = $carClass;
        $this->attributesClass = $attributesClass;
        $this->carTranslationClass = $carTranslationClass;
        $this->carTermClass = $carTermClass;
    ...
    public function restore($id)
    {
        ...
        $user_id = Auth::id();
        $query = $this->carClass::onlyTrashed()->where("author_id", $user_id)->where("id", $id)->first();
        if(!empty($query)){
            $query->restore();
    ...
    public function deleteCar($id)
    {
        ...
        if(\request()->query('permanently_delete')){
            $query = $this->carClass::where("author_id", $user_id)->where("id", $id)->withTrashed()->first();
            if (!empty($query)) {
                $query->forceDelete();
            }
        }else {
            $query = $this->carClass::where("author_id", $user_id)->where("id", $id)->first();
            if (!empty($query)) {
                $query->delete();
                event(new UpdatedServiceEvent($query));
            }
