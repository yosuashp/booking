...
@if(!empty($terms_ids) and !empty($attributes))
    @foreach($attributes as $attribute )
        ...
        @if(empty($attribute['parent']['hide_in_single']))
            <div class="g-attributes mb-4 {{$attribute['parent']->slug}} attr-{{$attribute['parent']->id}}">
                <h3 class="font-size-16">{{ $translate_attribute->name }}</h3>
                ...
                <div class="list-attributes">
                    @foreach($terms as $term )
                        ...
                        <div class="item {{$term->slug}} term-{{$term->id}}">
                            ...
                            @else
                                <i class="{{ $term->icon ?? "icofont-check-circled icon-default text-primary" }}"></i>
                            @endif
