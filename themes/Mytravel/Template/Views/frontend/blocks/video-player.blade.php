<div class="{{ str_replace('_','-',$bg_gradient ?? '') }} bg-img-hero space-3 space-top-lg-4 space-bottom-lg-3" @if(!empty($bg_image)) style="background-image: url({{get_file_url($bg_image,'full')}});" @endif>
    <div class="text-center mt-xl-2">
        ...
        @if(!empty($youtube))
        <!-- Fancybox -->
        <a class="travel-fancybox d-inline-block u-media-player" href="javascript:;" data-src="{{ handleVideoUrl($youtube) }}" data-speed="700" data-animate-in="zoomIn" data-animate-out="zoomOut" data-caption="{{$caption ?? ''}}">
